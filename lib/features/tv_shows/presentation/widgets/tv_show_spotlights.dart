...
class _SpotlightState extends State<Spotlight> {
  ...
  void initState() {
    super.initState();
    if (widget.tvShow.backdropPath != null && widget.tvShow.backdropPath!.isNotEmpty) {
      _colorSchemeFuture = ColorScheme.fromImageProvider(
        provider: CachedNetworkImageProvider(widget.tvShow.fullBackdropPath),
      );
    }
    _loadLogo();
  }
  ...
  void _loadLogo() {
    ...
    sl<TVShowRemoteDataSource>()
        .getTVShowDetails(widget.tvShow.id)
        .then((detail) {
          if (mounted) {
            setState(() {
  ...
  void didUpdateWidget(covariant Spotlight oldWidget) {
    ...
    if (widget.tvShow.id != oldWidget.tvShow.id) {
      _loadLogo();
    }
    if (widget.tvShow.backdropPath != oldWidget.tvShow.backdropPath) {
      if (widget.tvShow.backdropPath != null && widget.tvShow.backdropPath!.isNotEmpty) {
        setState(() {
          _colorSchemeFuture = ColorScheme.fromImageProvider(
            provider: CachedNetworkImageProvider(widget.tvShow.fullBackdropPath),
  ...
  Widget build(BuildContext context) {
    return Container(
      decoration: BoxDecoration(
        ...
        image: widget.tvShow.backdropPath != null && widget.tvShow.backdropPath!.isNotEmpty
            ? CineDecorationImage.cachedNetwork(widget.tvShow.fullBackdropPath)
            : null,
      ),
      child: Stack(
        children: [
    ...
    return Column(
      ...
      children: [
        ...
        Wrap(
          ...
          children: [
            _infoChip('TV SHOW'),
            if (widget.tvShow.voteAverage != 0.0)
              _infoItem('${widget.tvShow.voteAverage.toStringAsFixed(1)} ★', CineIcons.star),
        ...
        Row(
          children: [
            CinePrimaryBtn(
              ...
              scrollTillTop: true,
              onTap: () {
                context.push('/play/tv/${widget.tvShow.id}/1/1');
              },
            ),
            ...
            CineSecondaryBtn(
              ...
              scrollTillTop: true,
              onTap: () {
                context.push('/tv/${widget.tvShow.id}');
              },
            ),
          ],
        ),
...
class TVShowSpotlights extends StatefulWidget {
  final List<TVShow> tvShows;

  const TVShowSpotlights({super.key, required this.tvShows});

  @override
  TVShowSpotlightsState createState() => TVShowSpotlightsState();
}

class TVShowSpotlightsState extends State<TVShowSpotlights> {
  ...
  void initState() {
    ...
    _watchNowFocusNodes = List.generate(
      ...
      (index) => FocusNode(debugLabel: 'TVWatchNow $index')..onKeyEvent = _onWatchNowKeyEvent,
    );
    _moreInfoFocusNodes = List.generate(
      widget.tvShows.length,
      (index) => FocusNode(debugLabel: 'TVMoreInfo $index')..onKeyEvent = _onMoreInfoKeyEvent,
    );
  ...
  @override
  void didUpdateWidget(covariant TVShowSpotlights oldWidget) {
    super.didUpdateWidget(oldWidget);
  ...
  void next() async {
    ...
    if (mounted) {
      ...
      final int lastIndex = widget.tvShows.length - 1;
        ...
        Focus(
          ...
          child: PageView.builder(
            ...
            itemBuilder: (BuildContext context, int index) {
              final data = widget.tvShows[index];
              return Spotlight(
